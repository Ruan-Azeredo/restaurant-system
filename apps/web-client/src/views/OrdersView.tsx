      ...
      <CardHeader className="p-6 pb-4">
        <div className="flex items-start justify-between gap-4">
          ...
          <Badge
            ...
            className={cn(
              "px-4 py-1 text-[10px] font-black uppercase tracking-widest shadow-lg shadow-white/5",
              status === "pending" &&
    ...
    <div className="px-6 py-10 max-w-5xl mx-auto w-full space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6">
        ...
        <Button
          ...
          className="gap-2 px-6 font-bold bg-background/50 backdrop-blur-sm border-white/10 hover:bg-background/80 shadow-sm transition-all active:scale-95"
        >
          <RefreshCw
            className={cn("size-4 stroke-[2.5]", loading && "animate-spin")}
          />
          Refresh Status
        </Button>
      </div>
      ...
      {!clientId ? (
        ...
        <Tabs defaultValue="active" className="space-y-8">
          <TabsList className="bg-muted/30 p-1 w-fit backdrop-blur-sm border border-white/5">
            <TabsTrigger
              value="active"
              className="px-6 py-2 transition-all data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-lg"
            >
              Active Orders
